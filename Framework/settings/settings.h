...
// Replace D3DX11tex.h with DirectXTex
...
public:
    ...
    // Function to load texture using DirectXTex
    bool LoadTexture(ID3D11Device* device, const std::wstring& filePath)
    {
        ...
        hr = DirectX::CreateShaderResourceView(device, image.GetImages(), image.GetImageCount(), image.GetMetadata(), &c_texture.background);
        return SUCCEEDED(hr);
    }
};
...
public:
    ...
    struct
    {
        ImVec4 accent_clr = ImColor(255, 0, 0, 222);

        ImVec4 white_clr = ImColor(255, 255, 255, 255);
        ImVec4 black_clr = ImColor(12, 12, 12, 255);
    ...
    struct
    {
        ImVec4 layout = ImColor(0, 0, 0, 255);
        ImVec4 border = ImColor(0, 0, 0, 255);
