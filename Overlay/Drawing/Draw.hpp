...
namespace FontList
{
	...
	inline ImFont* Pixel;
	inline ImFont* consola;
	inline ImFont* verdana;
	inline std::vector<std::string> FontText = { "Arial", "Calibri", "Ebrima", "Franklin", "Pixel", "Consola", "verdana"};
...
namespace Draw {
	...
	public:
		...
		ImVec2 Position = { 25,25 };
		 void CustomSliderFloat(const char* label, float* v, float v_min, float v_max, const char* format = _("%.0f"));
		 void CustomSliderInt(const char* label, int* v, int v_min, int v_max, const char* format = _("%.0d"));
